<%@ WebService Language="C#" Class="WS_StaffWise" %>
...
public class WS_StaffWise  : System.Web.Services.WebService {
    public static CultureInfo ci = new CultureInfo("en-GB");

    [WebMethod(EnableSession=true)]
    public string Get_All_Staff_Project_Job_BranchName(int compid=0)
    {
        CommonFunctions objComm = new CommonFunctions();
        List<Staffwise> List_SM = new List<Staffwise>();
        ...
        IEnumerable<Staffwise> tbl = List_SM as IEnumerable<Staffwise>;
        return new JavaScriptSerializer().Serialize(tbl);
    }

    [WebMethod(EnableSession=true)]
    public string Get_Staffwise_Project_Job_All_Selected(StaffProject currobj)//int compid, string UserType, string status, string StaffCode)
    {
        ...
        try
        {
            if (Session["companyid"] != null)
            {
                ...
                // DataSet ds = SqlHelper.ExecuteDataset(objComm._cnnString, CommandType.StoredProcedure, "Usp_Get_Staffwise_Project_Job_All_Selected", param);
                using (SqlDataReader drrr = SqlHelper.ExecuteReader(objComm._cnnString, CommandType.StoredProcedure, "Usp_Get_Staffwise_Project_Job_All_Selected", param))
                {
                    while (drrr.Read())
                    {
                        List_SM.Add(new Staffwise()
