<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AddTaskJobname.ascx.cs" Inherits="controls_AddTaskJobname" %>
...
//////////hide modalpopup
    ...
    function getTask(pageIndex, pageNewSize) {
        ...
        try {
            var calala = $.ajax({
                type: "POST",
                url: "../Handler/wsJobName.asmx/bind_TaskJobname",
                ...
                success: function (msg) {
                    ...
                    if (myList.length > 0) {
                        ...
                        for (var i = 0; i < myList.length; i++) {
                            tr = '<tr><td>' + parseFloat(i + 1) + '</td>';
                            tr = tr + '<td>' + myList[i].Task_name + '<input type="hidden" id="hdnTskid" name="hdnTskid" value=' + myList[i].Task_Id + '></td>';
                            if (l == '1') {
                                tr = tr + '<td>' + myList[i].mjobName + '</td>';

                            }
                            tr = tr + '<td><img id="btnEdit" src="../images/edit.png" onclick="ShowEdit($(this))"/></td>';
                            tr = tr + '<td><img id="btnDel" src="../images/Delete.png" onclick="ShowDelete($(this))"/></td>';
                            tr = tr + '</tr>';
                            $('#tblTsk').append(tr);
                        }
                        $('.loader').hide();
    ...
    function getProject(tskid) {
        ...
        try {
            var calala = $.ajax({
                type: "POST",
                url: "../Handler/wsJobName.asmx/bind_Task_JobName",
                ...
                success: function (msg) {
                    ...
                    for (var i = 0; i < myList.length; i++) {
                        ...
                        tr = $('<tr><td><input type="hidden" name="hdnjid" value="' + myList[i].mjobid + '"><input type="checkbox" id="chkjb" name="chkjb" value="' + myList[i].mjobid + '"' + vadfds + ' />' + myList[i].mjobName + '</td></tr>');
    ...
    function Save_Task(jid, t, tid) {
        ...
        try {
            var calala = $.ajax({
                type: "POST",
                url: "../Handler/wsJobName.asmx/TaskJobname_Save",
                data: JSON.stringify(data),
                dataType: 'json',
                contentType: "application/json",
                success: function (msg) {
...
<%--/////pageload controlls and grid--%>
    ...
    <asp:Panel ID="panel10" runat="server" Width="421px" BackColor="#FFFFFF">
      <div id="Div1" style="width: 100%; float: left; background-color: #55A0FF; color: #ffffff;
        ...
        <div id="divJb" class="divedBlock">
            ...
            <div style="overflow: hidden; width:400px; height:300px; float: left; padding-left: 5px;">
              <asp:Panel ID="pnljb" runat="server" ScrollBars="Vertical" Height="280px" Width ="380px" >
                    <table id="tblJb" >
                    </table> 
              </asp:Panel>
            </div>   
