<%@ WebService Language="C#" Class="MinimumHrsEffort" %>
...
public class MinimumHrsEffort : System.Web.Services.WebService
{
    CommonLibrary.CommonFunctions objComm = new CommonLibrary.CommonFunctions();
    [WebMethod(EnableSession = true)]
    public string GetStaffEffortMinimum(string depId, string month, string status, int timesheetCondition)
    {
        string monthDate = month != "" ? Convert.ToDateTime(month).ToString("MM/dd/yyyy") : null;
        List<tbl_Staff_Effort> List_DS = new List<tbl_Staff_Effort>();
        tbl_Staff_Effort dayList = new tbl_Staff_Effort();
        List<tbl_Staff_Effort> filteredTimesheet = new List<tbl_Staff_Effort>();
        DataSet dds = new DataSet();
        DataTable dtWorkingDays;
        tbl_MinimumHrsEffort departmentwise_Staff_MinHrs_Effort_Report = new tbl_MinimumHrsEffort();
        try
        {
                SqlParameter[] param = new SqlParameter[5];
                param[0] = new SqlParameter("@CompId", Session["companyid"]);
                param[1] = new SqlParameter("@DeptId", depId);
                param[2] = new SqlParameter("@SearchMonth", monthDate);
                param[3] = new SqlParameter("@Status", status);
                param[4] = new SqlParameter("@TimesheetCondition", timesheetCondition);

                using (SqlDataReader drrr = SqlHelper.ExecuteReader(objComm._cnnString, CommandType.StoredProcedure, "usp_Bootstrap_GetMinimumEffort_Report", param))
                {
                    while (drrr.Read())
                    {
                        List_DS.Add(new tbl_Staff_Effort()
                        {
                            staffBioServerid = objComm.GetValue<string>(drrr["StaffCode"].ToString()),
                            ...
                            DateOfJoining = drrr["DateOfJoining"] == null ? null : objComm.GetValue<string>(drrr["DateOfJoining"].ToString()),
                            DateOfLeaving = drrr["DateOfLeaving"] == null ? null : objComm.GetValue<string>(drrr["DateOfLeaving"].ToString()),
                            EmpType = drrr["EmpType"] == null ? null : objComm.GetValue<string>(drrr["EmpType"].ToString()),
                            DepartmentName = drrr["DepartmentName"] == null ? null : objComm.GetValue<string>(drrr["DepartmentName"].ToString()),
                        });
                    }
                    if (drrr.NextResult())
                    {
                        if (drrr.HasRows)
                        {
                            while (drrr.Read())
                            {
                                departmentwise_Staff_MinHrs_Effort_Report.CompanyThresold = objComm.GetValue<decimal>(drrr["CompanyThresold"].ToString());
                            }
                        }
                    }

                    dds = SqlHelper.ExecuteDataset(objComm._cnnString, CommandType.StoredProcedure, "usp_Bootstrap_GetMinimumEffort_Report", param);
                    dtWorkingDays = dds.Tables[2];
                }
        ...
        if (timesheetCondition == 0)
        {
            departmentwise_Staff_MinHrs_Effort_Report.tbl_Staff_Effort = List_DS;
        }
        else
        {
            foreach (var item in List_DS)
            {
                var defualtedTimesheet = CheckAndGetDefualtedTimesheet(item, dtWorkingDays);
                if (defualtedTimesheet != null)
                    filteredTimesheet.Add(defualtedTimesheet);

                departmentwise_Staff_MinHrs_Effort_Report.tbl_Staff_Effort = filteredTimesheet;
            }
        }

        return new JavaScriptSerializer().Serialize(departmentwise_Staff_MinHrs_Effort_Report);
    }

    public tbl_Staff_Effort CheckAndGetDefualtedTimesheet(tbl_Staff_Effort timesheet, DataTable dtWorkingDays)
    {
        string dayPropertyName = "Day";

        foreach (DataRow day in dtWorkingDays.Rows)
        {
            object value = timesheet.GetType().GetProperty(dayPropertyName + day["DayNumber"].ToString()).GetValue(timesheet, null);

            if (!string.IsNullOrEmpty(value.ToString()))
            {
                if (Convert.ToDecimal(value.ToString()) < 8)
                    return timesheet;
            }
    ...
    [WebMethod(EnableSession=true)]
    public string GetStaffsLeaves(string searchMonth, string staffCodes)
    {
        CommonFunctions objComm = new CommonFunctions();
