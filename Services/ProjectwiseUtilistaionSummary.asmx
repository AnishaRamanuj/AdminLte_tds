<%@ WebService Language="C#" Class="ProjectwiseUtilistaionSummary" %>
...
public class ProjectwiseUtilistaionSummary : System.Web.Services.WebService
{
    CommonLibrary.CommonFunctions objComm = new CommonLibrary.CommonFunctions();

    [WebMethod(EnableSession=true)]
    public string GetClientProjectReport(int depId, string month)
    {
        string monthDate = month != "" ? Convert.ToDateTime(month).ToString("MM/dd/yyyy") : null;
        List<tbl_ClientProjectReport> List_DS = new List<tbl_ClientProjectReport>();
        try
        {
            if (Session["companyid"] != null)
            {
                ...
                using (SqlDataReader drrr = SqlHelper.ExecuteReader(objComm._cnnString, CommandType.StoredProcedure, "usp_bootstrap_Project_Utilisation_Summary_Report", param))
                {
                    while (drrr.Read())
                    {
                        List_DS.Add(new tbl_ClientProjectReport()
                        {
                            Client = objComm.GetValue<string>(drrr["Client"].ToString()),
                            Project = objComm.GetValue<string>(drrr["Project"].ToString()),
                            ScopeofWork = objComm.GetValue<string>(drrr["ScopeofWork"].ToString()),
