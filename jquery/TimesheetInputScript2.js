...
$(document).ready(function () {
    ...
    if (main_obj.length == 0)
        ServerServiceToGetData('{compid:' + $("[id*=hdnCompanyID]").val() + ',staffcode:' + $("[id*=hdnStaffCode]").val() + ',inputType:"' + $("[id*=hdnTimesheetInputType]").val() + '"}', '../Handler/wsTimesheetInput2.asmx/getTimesheetInpurtrelatedDetails', false, Onsuccess_client_projct_job_details);
...
function saveTimesheets(status) {
    ...
    $('.modalganesh').css('display', 'block');
    ServerServiceToGetData(saveobj, '../Handler/wsTimesheetInput2.asmx/savetimesheetinput', false, Onsuccess_saveTimesheets);
}
...
function getTimesheetsofSelectedWeek() {
    if ($("[id*=txtdate]").val() != "" && currentWeek.length == 7) {
        ...
        ServerServiceToGetData(sendObj, '../Handler/wsTimesheetInput2.asmx/getTimesheetsofSelectedWeek', false, Onsuccess_getSavedTimesheets);
    }
}
...
function deletesavedTimesheet(tsid) {
    ...
    if (con == true) {
        ...
        ServerServiceToGetData(sendObj, '../Handler/wsTimesheetInput2.asmx/deletesavedTimesheet', false, Onsuccess_timesheetdeleted);
    }
}
//////deleted success
...
function editTsExpense(cuurTsid, cuurojb) {
    ...
    { Onsuccess_ExpenseEdit(null); }
        ...
        ServerServiceToGetData(sendObj, '../Handler/wsTimesheetInput2.asmx/getExpenseagiainsTSID', false, Onsuccess_ExpenseEdit);
    }
}
/////onexpense edit
...
function saveExpesneOnEdittsidbased(currTsId) {
    ...
    ServerServiceToGetData(sendObj, '../Handler/wsTimesheetInput2.asmx/saveExpenseagiainsTSID', false, Onsuccess_ExpenseEditSave);
}
...
function saveEditTimesheetInput(status) {
    ...
    ServerServiceToGetData(ts, '../Handler/wsTimesheetInput2.asmx/saveeditSaveTimesheetInput', false, Onsuccess_saveeditTimesheets);
}
...
function Submitforapproval() {
    ...
    ServerServiceToGetData(sendObj, '../Handler/wsTimesheetInput2.asmx/SubmitSavedTiemsheets', false, Onsuccess_Submitforapproval);
}
